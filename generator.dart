...
class Generate {
  static void createFeature() async {
    ...
    String viewFolder = '${featuresPath}screen/';
    ...
    /// * Update Navigation
    updateNavigation(name: featureName);
    updatePageNavigation(name: featureName);
    updatePageImportNavigation(
  ...
  static void createSubFeature() {
    ...
    file(
      ...
      content: GeneratorModuleData.binding(
        controllerPath: controllerPath,
        nameFeature: featureName,
        nameClassScreen: moduleNamePascalCase,
      ),
    );
  ...
  static Future<void> updatePageNavigation({
    required String name,
  }) async {
    String pascalCaseToCamelCase = DataConverter.pascalCaseToCamelCase(name);
    String camelCaseToSnakeCase = DataConverter.camelCaseToSnakeCase(name);

    var route = File("${configsPath}pages/page.dart");

    var readDataRoute = await route.readAsString();

    var contentRoute = readDataRoute.replaceAll("];", """GetPage(
      name: Routes.${pascalCaseToCamelCase}Route,
      page: () => const ${name}Screen(),
  ...
  static Future<void> updatePageImportNavigation({
    required String nameFeature,
    required String bindingPath,
    required String screenPath,
  }) async {
    String moduleNameSnakeCase =
        DataConverter.camelCaseToSnakeCase(nameFeature);
    String importBindingPath =
        "${getImportCurPackage()}/$bindingPath${moduleNameSnakeCase}_binding.dart"
            .replaceAll("/lib/", "/");
    String importScreenPath =
        "${getImportCurPackage()}/$screenPath${moduleNameSnakeCase}_screen.dart"
            .replaceAll("/lib/", "/");
    var route = File("${configsPath}pages/page.dart");
    var lines = await route.readAsLinesSync();
    int lastImportIndex =
...
$importScreenPath';
""";
    // Cek jika import sudah ada
    final alreadyExists = lines.contains(contentImport);
    if (!alreadyExists) {
      // Sisipkan import baru setelah import terakhir
      lines.insert(lastImportIndex + 1, contentImport);
      // Tulis ulang file
      route.writeAsStringSync(lines.join('\n'));
    }
  }
